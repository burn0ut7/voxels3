...
PS
{
	...
	TerrainSample SampleTerrainProjection( Texture2D colorMap, Texture2D normalMap, Texture2D surfaceMap, float2 uv, float2 gradientX, float2 gradientY, float contribution )
	{
	...
	TerrainSample SampleTerrain( Texture2D colorMap, Texture2D normalMap, Texture2D surfaceMap,
		...
		float3 gradientY, float3 normal, float3 projection )
	{
		TerrainSample result = (TerrainSample)0;
		if ( weight <= 0.0 )
		{
		...
		gradientX *= scale;
		gradientY *= scale;
		TerrainSample sampleX = SampleTerrainProjection( colorMap, normalMap, surfaceMap,
			position.yz, gradientX.yz, gradientY.yz, projection.x );
		TerrainSample sampleY = SampleTerrainProjection( colorMap, normalMap, surfaceMap,
			position.xz, gradientX.xz, gradientY.xz, projection.y );
		TerrainSample sampleZ = SampleTerrainProjection( colorMap, normalMap, surfaceMap,
	...
	float4 MainPs( PixelInput input ) : SV_Target0
	{
		...
		TerrainSample grass = SampleTerrain( GrassColorMap, GrassNormalMap, GrassSurfaceMap,
			1.4, weights.x, position, gradientX, gradientY, normal, projection );
		...
		TerrainSample dirt = SampleTerrain( DirtColorMap, DirtNormalMap, DirtSurfaceMap,
			2, weights.y, position, gradientX, gradientY, normal, projection );
		...
		TerrainSample stone = SampleTerrain( StoneColorMap, StoneNormalMap, StoneSurfaceMap,
			2.38, weights.z, position, gradientX, gradientY, normal, projection );
		...
		TerrainSample snow = SampleTerrain( SnowColorMap, SnowNormalMap, SnowSurfaceMap,
			1.0, weights.w, position, gradientX, gradientY, normal, projection );
		...
		TerrainSample sand = SampleTerrain( SandColorMap, SandNormalMap, SandSurfaceMap,
			1.0, sandWeight, position, gradientX, gradientY, normal, projection );
