...
internal sealed partial class GpuVoxelMesher : IDisposable
{
	...
	private DrawCommandCommitResult CommitDrawCommandsLocked( RenderCameraState state )
	{
		...
		if ( _visibilityCapacity > 0 && _fieldPresentationReady )
		{
			...
			_voxelMaterials.Bind( commands.Attributes );
			...
			foreach ( var arena in _arenas )
			{
				...
				drawAttributes.Set( "VoxelDepthVertices", arena.Vertices );
				drawAttributes.Set( "VoxelDepthVertexOffset", 0 );
				commands.Attributes.Set( "VoxelDepthVertices", arena.Vertices );
				commands.Attributes.Set( "VoxelDepthVertexOffset", 0 );
				commands.ResourceBarrierTransition( arena.Vertices, ResourceState.GenericRead );
	...
	private void SetVisibilityActiveLocked( ResidentMesh resident, bool active )
	{
		...
		SetDepthVisibility( resident.Handle, active,
			new BBox( origin - padding, origin + new Vector3( size ) + padding ) );
	...
	private void SetTransitionVisibilityActiveLocked( ResidentTransition resident, bool active )
	{
		...
		SetDepthVisibility( resident.Handle, active,
			new BBox( minimum - new Vector3( padding ), maximum + new Vector3( padding ) ) );
		_sourceArgumentData[slot] = active
			? new GpuBuffer.IndirectDrawIndexedArguments
			{
				IndexCount = (uint)resident.Handle.Indices.Count,
	...
	private void SetDepthVisibility( GeometryHandle handle, bool active, BBox bounds )
	{
		lock ( handle )
		{
			if ( active && handle.DepthObject is null )
				handle.DepthObject = new TerrainDepthObject( _scene.SceneWorld, this, handle, bounds );
			if ( handle.DepthObject is not null ) handle.DepthObject.RenderingEnabled = active;
		}
	}
	...
	{
		public TerrainDepthObject DepthObject { get; set; }
	...
	private sealed class TerrainDepthObject : SceneCustomObject
	{
		...
		public TerrainDepthObject( SceneWorld world, GpuVoxelMesher owner, GeometryHandle handle, BBox bounds ) : base( world )
		{
			_owner = owner;
			_handle = handle;
			Bounds = bounds;
			...
			Attributes.Set( "VoxelDepthVertices", handle.Arena.Vertices );
			Attributes.Set( "VoxelDepthVertexOffset", handle.Vertices.Offset );
			Attributes.Set( "VoxelMaterialPalette", owner._voxelMaterials.Palette );
		}
		...
		public override void RenderSceneObject()
		{
			...
			lock ( _handle )
			{
				if ( _handle.DepthObject != this || !RenderingEnabled || !_owner._fieldPresentationReady ) return;
				Graphics.ResourceBarrierTransition( _handle.Arena.Vertices, ResourceState.GenericRead );
				Graphics.ResourceBarrierTransition( _handle.Arena.Indices, ResourceState.VertexOrIndexBuffer );
				Graphics.Draw( _handle.Arena.Vertices, _handle.Arena.Indices, _owner._material,
					_handle.Indices.Offset, _handle.Indices.Count, Attributes );
			}
		}
	}
