...
VS
{
		...
		StructuredBuffer<TerrainDepthVertex> VoxelDepthVertices < Attribute( "VoxelDepthVertices" ); >;
		uint VoxelDepthVertexOffset < Attribute( "VoxelDepthVertexOffset" ); >;
	#endif
	...
	PixelInput MainVs( const VertexInput input, uint vertexId : SV_VertexID )
	{
		PixelInput output = (PixelInput)0;
		float3 position = input.Position;
		float2 encodedNormal = input.Normal.yz;
		#if ( S_MODE_DEPTH )
			// Public scene draws do not expose a base-vertex argument.
			TerrainDepthVertex vertex = VoxelDepthVertices[vertexId + VoxelDepthVertexOffset];
			position = asfloat( vertex.First.xyz );
			encodedNormal = asfloat( vertex.Second.xy );
		#endif
		float3 normal = DecodeTerrainNormal( encodedNormal );
