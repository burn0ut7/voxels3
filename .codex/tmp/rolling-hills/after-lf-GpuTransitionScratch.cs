...
internal sealed class GpuTransitionScratch : IDisposable
{
	...
	private readonly GpuBuffer<float> _densitySamples = new(
		DensityCount * MaximumBatchSize * 2, GpuBuffer.UsageFlags.Structured, "Voxel Transition Scratch Density" );
		...
		if ( fields is not null )
		{
			...
			for ( var block = 0; block < count; block++ )
			{
				...
				if ( field.Pages.Values.Any( page => page.HasMaterials ) )
					for ( var index = 0; index < DensityCount; index++ )
						_correctionUpload[index] = field.SamplePlacedDirt( DensitySamplePosition( request, index ) );
				_densitySamples.SetData<float>( _correctionUpload.AsSpan(), DensityCount * MaximumBatchSize + block * DensityCount );
			}
		}
		SetBatchSize( count );
	...
	private void BindCommonAttributes()
	{
		...
		_shader.Attributes.Set( "TransitionPlacedMaterialOffset", DensityCount * MaximumBatchSize );
