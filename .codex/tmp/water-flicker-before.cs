...
public sealed partial class VoxelManager
{
	...
	private long _waterObservedPlacement = -1;
	private bool _waterObservedPending;
	...
	{
		if ( !_levels[0].HasPlacement ) return;
	...
	private void RefreshWaterCellRequests()
	{
		...
		if ( _waterObservedSerial == _waterRequestSerial && _waterObservedFieldRevision == field.Revision &&
			_waterObservedFieldEpoch == field.Epoch && _waterObservedPlacement == _clipboxPlacementCommits &&
			_waterObservedPending == _clipboxPlacementPending ) return;
		_waterCellRequests.Clear();
		foreach ( var state in _levels )
		{
			var cellSize = CellSizeForLevel( state.Level );
			var size = _appliedCellsPerAxis * cellSize;
			var z = (int)MathF.Ceiling( field.Settings.SeaLevel / size ) - 1;
			for ( var staged = 0; staged < 2; staged++ )
			{
				if ( staged == 0 && (!state.HasPlacement || !state.VisualEnabled) ) continue;
				if ( staged == 1 && (!_clipboxPlacementPending || (state.Level < _stagedVisualConfiguration.MinimumVisualLod || state.Level > _stagedVisualConfiguration.MaximumVisualLod)) ) continue;
				var active = staged == 1 && state.PlacementChanged ? state.NextActive : state.Active;
				foreach ( var coordinate in active )
				{
					...
					_waterCellRequests[descriptor] = new SdfWorldAabb( minimum, minimum + Vector3.One * size );
				}
			}
		}
		foreach ( var key in _waterCells.Keys.Where( key => !_waterCellRequests.ContainsKey( key ) ).ToArray() ) _waterCells.Remove( key );
		_waterObservedSerial = _waterRequestSerial;
		_waterObservedFieldRevision = field.Revision;
		_waterObservedFieldEpoch = field.Epoch;
		_waterObservedPlacement = _clipboxPlacementCommits;
		_waterObservedPending = _clipboxPlacementPending;
		_waterCellRevision++;
	}
	...
	private void UpdateWaterCellGeneration()
	{
		...
		if ( _waterCellPreparation is not null )
		{
			...
			foreach ( var item in _waterCellPreparation.GetAwaiter().GetResult() )
			{
		...
		if ( _waterCells.Count == _waterCellRequests.Count ) return;
		var work = new List<(GpuSdfDescriptor Descriptor, SdfWorldAabb Bounds, TerrainFieldSnapshot Field)>();
		foreach ( var pair in _waterCellRequests )
		{
			if ( _waterCells.ContainsKey( pair.Key ) ) continue;
			if ( !CurrentField.TryCaptureRegion( pair.Key.SamplingBounds, out var field ) ) continue;
			work.Add( (pair.Key, pair.Value, field) );
			if ( work.Count == 8 ) break;
		}
		if ( work.Count == 0 ) return;
		_waterCellCancellation ??= new CancellationTokenSource();
		var cancellation = _waterCellCancellation.Token;
		_waterCellPreparation = GameTask.RunInThreadAsync( async () =>
		{
			var result = new SurfaceWaterGeometry.Chunk[work.Count];
			for ( var index = 0; index < work.Count; index++ )
			{
				...
				var cells = GeneratedWaterCells.Generate( item.Bounds, item.Descriptor.CellSize, item.Field, cancellation );
				var vertices = SurfaceWaterGeometry.Build( cells );
				result[index] = new SurfaceWaterGeometry.Chunk( item.Descriptor, cells, vertices, System.Diagnostics.Stopwatch.GetElapsedTime( started ).TotalMilliseconds );
	...
	private void UpdateSurfaceWaterChunks()
	{
		...
		if ( _waterPlacementRevision == _clipboxPlacementCommits && _waterResidentRevision == residentRevision &&
			_waterRenderedCellRevision == _waterCellRevision ) return;
		_nextWaterChunks.Clear();
		foreach ( var pair in _waterCells )
		{
			var descriptor = pair.Key;
			var state = _levels[descriptor.Key.Level];
			if ( state.HasPlacement && state.VisualEnabled && state.Active.Contains( descriptor.Key.Coordinate ) &&
				_gpuMesher.IsResident( descriptor ) ) _nextWaterChunks.Add( pair.Value );
		}
