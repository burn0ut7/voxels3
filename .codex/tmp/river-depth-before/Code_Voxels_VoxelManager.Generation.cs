		...
		float minimumY = -131072f, int pointsPerAxis = 65, float spacing = 4096f )
	{
		...
		try
		{
			return await Task.RunInThreadAsync( () =>
			{
				...
				var riverText = new System.Text.StringBuilder( "startX,startY,startZ,endX,endY,endZ,startRadius,endRadius\n" );
				...
				foreach ( var patch in riverPatches )
				{
					...
					foreach ( var segment in RiverWorld.For( settings ).GetPatch( patch ).Segments )
					{
						...
						riverText.AppendLine( FormattableString.Invariant(
							$"{segment.Start.x:R},{segment.Start.y:R},{segment.Start.z:R},{segment.End.x:R},{segment.End.y:R},{segment.End.z:R},{segment.StartRadius:R},{segment.EndRadius:R}" ) );
					}
