...
PS
{
	...
	// Intermediate height queries need less anisotropy than the final shaded maps.
	SamplerState TerrainHeightSampler < Filter( ANISOTROPIC ); MaxAniso( 4 ); AddressU( WRAP ); AddressV( WRAP ); >;
	float2 TerrainParallax( Texture2D surfaceMap, float reliefRatio, float2 uv, float2 gradientX, float2 gradientY, float3 view, float strength )
	{
		...
		float previousGap = 1.0 - surfaceMap.SampleGrad( TerrainHeightSampler, uv, gradientX, gradientY ).b;
		if ( previousGap <= 0.0 )
		{
			return uv;
		}
		...
		for ( int step = 1; step <= 24; step++ )
		{
			...
			float surfaceDepth = 1.0 - surfaceMap.SampleGrad( TerrainHeightSampler, currentUv, gradientX, gradientY ).b;
