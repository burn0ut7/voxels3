...
PS
{
	...
	TerrainSample SampleTerrain( Texture2D colorMap, Texture2D normalMap, Texture2D surfaceMap,
		float tileMetres, float reliefMetres, float normalStrength, float weight, float3 position, float3 gradientX,
		float3 gradientY, float3 normal, float3 projection, float3 view, float reliefFade )
	{
		...
		// Scale tangent slopes consistently with authored height before whiteout.
		sampleX.Normal.xy *= normalStrength;
		sampleY.Normal.xy *= normalStrength;
		sampleZ.Normal.xy *= normalStrength;
	...
	float4 MainPs( PixelInput input ) : SV_Target0
	{
		...
		TerrainSample grass = SampleTerrain( GrassColorMap, GrassNormalMap, GrassSurfaceMap,
			1.4, 0.02, 1.0, weights.x, position, gradientX, gradientY, normal, projection, view, reliefFade );
		...
		TerrainSample dirt = SampleTerrain( DirtColorMap, DirtNormalMap, DirtSurfaceMap,
			2, 0.045, 1.35, weights.y, position, gradientX, gradientY, normal, projection, view, reliefFade );
		...
		TerrainSample stone = SampleTerrain( StoneColorMap, StoneNormalMap, StoneSurfaceMap,
			2.38, 0.04, 1.0, weights.z, position, gradientX, gradientY, normal, projection, view, reliefFade );
		...
		TerrainSample snow = SampleTerrain( SnowColorMap, SnowNormalMap, SnowSurfaceMap,
			1.0, 0.003, 1.0, weights.w, position, gradientX, gradientY, normal, projection, view, reliefFade );
		...
		TerrainSample sand = SampleTerrain( SandColorMap, SandNormalMap, SandSurfaceMap,
			1.0, 0.002, 1.0, sandWeight, position, gradientX, gradientY, normal, projection, view, reliefFade );
