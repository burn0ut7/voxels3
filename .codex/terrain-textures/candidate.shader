...
PS
{
	...
	float4 MainPs( PixelInput input ) : SV_Target0
	{
		...
		TerrainSample dirt = SampleTerrain( DirtColorMap, DirtNormalMap, DirtSurfaceMap,
			2, 0.018, weights.y, position, gradientX, gradientY, normal, projection, view, reliefFade );
		material.Albedo += dirt.Color * weights.y;
		material.Normal += dirt.Normal * weights.y;
		material.Roughness += lerp( 0.8, 1.0, saturate( dirt.Surface.r ) ) * weights.y;
		...
		TerrainSample snow = SampleTerrain( SnowColorMap, SnowNormalMap, SnowSurfaceMap,
			2, 0.025, weights.w, position, gradientX, gradientY, normal, projection, view, reliefFade );
		...
		TerrainSample sand = SampleTerrain( SandColorMap, SandNormalMap, SandSurfaceMap,
			1.5, 0.012, sandWeight, position, gradientX, gradientY, normal, projection, view, reliefFade );
