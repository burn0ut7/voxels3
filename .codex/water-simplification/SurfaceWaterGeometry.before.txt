...
internal static class SurfaceWaterGeometry
{
	...
	public static WaterVertex[] Build( GeneratedWaterCells cells )
	{
		...
		for ( var x = 0; x < size; x++ )
		{
			...
			if ( coverage[x + size * y] == 1 )
			{
				...
				AddTriangle( first, right, diagonal, vertices );
				AddTriangle( first, diagonal, up, vertices );
				continue;
			}
	...
	private static void ClipCell( Vector3 origin, float size, Vector4 wet, List<WaterVertex> vertices )
	{
		...
		{
			AddTriangle( clipped[0], clipped[index], clipped[index + 1], vertices );
		}
	}

	private static void AddTriangle( Vector3 a, Vector3 b, Vector3 c, List<WaterVertex> vertices )
	{
		var ab = b - a;
		var ac = c - a;
		if ( ab.x * ac.y - ab.y * ac.x == 0f ) return;
		vertices.Add( new WaterVertex( a ) );
		vertices.Add( new WaterVertex( b ) );
		vertices.Add( new WaterVertex( c ) );
	}
