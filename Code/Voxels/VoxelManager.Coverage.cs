...
{
	private bool _coverageTraceEnabled;
	private float _coverageTraceElapsed;
	private readonly CoverageTraceSample[] _coverageTrace = new CoverageTraceSample[256];
	private int _coverageTraceCount;
	private int _coverageTraceCursor;
	private long _coverageTraceSamples;
	private long _coverageTraceMissingSamples;
	private int _coverageTracePreviousMissing;
	private double _coverageTraceMaximumMilliseconds;
	private long _coverageTraceStarted = Stopwatch.GetTimestamp();

	private readonly record struct CoverageTraceSample( double Seconds, Vector3Int Center,
		int MissingCells, int AssignedWithoutResident, int FineCells, int CoarseCells,
		Vector3Int FirstMissing, int LargestUncoveredLod,
		bool RootHasPartialCoverage, bool LayoutPending, bool RefinementPending );

	// Sample coverage occupancy, not terrain density. An uncovered air cell is not
	// proof of a visible hole. Fixed storage and arithmetic ancestor lookups keep
	// this recorder independent of world size and avoid per-frame allocations.
	private void UpdateCoverageTrace()
	{
		if ( !_coverageTraceEnabled || !_levels[0].HasPlacement ) return;
		_coverageTraceElapsed += RealTime.Delta;
		if ( _coverageTraceElapsed < 0.25f ) return;
		_coverageTraceElapsed = 0f;
		var started = Stopwatch.GetTimestamp();
		var center = WorldToChunkCoordinate( ActiveStreamingTarget.WorldPosition );
		var missing = 0;
		var assignedWithoutResident = 0;
		var fine = 0;
		var coarse = 0;
		var firstMissing = default(Vector3Int);
		var largest = -1;
		var partial = false;
		for ( var z = -1; z <= 1; z++ )
		for ( var y = -1; y <= 1; y++ )
		for ( var x = -1; x <= 1; x++ )
		{
			var coordinate = center + new Vector3Int( x, y, z );
			var key = new GpuMeshRegionKey( 0, coordinate );
			if ( FindCoverageAncestor( key, out var active ) )
			{
				if ( !_gpuMesher.IsResident( active ) ) assignedWithoutResident++;
				if ( active.Level == 0 ) fine++; else coarse++;
				continue;
			}
			if ( missing++ != 0 ) continue;
			firstMissing = coordinate;
			var maximum = _targetVisualConfiguration.MaximumVisualLod;
			partial = _gpuMesher.HasRenderDescendant( CoverageAncestor( key, maximum ) );
			for ( var level = 0; level <= maximum; level++ )
			{
				if ( HasCoverageDescendant( CoverageAncestor( key, level ) ) ) break;
				largest = level;
			}
		}
		var sample = new CoverageTraceSample( Stopwatch.GetElapsedTime( _coverageTraceStarted ).TotalSeconds,
			center, missing, assignedWithoutResident, fine, coarse, firstMissing, largest, partial, _clipboxPlacementPending,
			_coveragePending.Count == 1 && _coveragePending[0].Split );
		_coverageTrace[_coverageTraceCursor] = sample;
		_coverageTraceCursor = (_coverageTraceCursor + 1) % _coverageTrace.Length;
		_coverageTraceCount = Math.Min( _coverageTraceCount + 1, _coverageTrace.Length );
		_coverageTraceSamples++;
		if ( missing > 0 ) _coverageTraceMissingSamples++;
		if ( (missing > 0) != (_coverageTracePreviousMissing > 0) )
			Log.Info( $"[VoxelWorld] coverage.gap {(missing > 0 ? "begin" : "end")} " +
				$"center={center} missing={missing}/27 first={firstMissing} coarseCandidateLod={largest} " +
				$"partialRoot={partial} pendingLayout={_clipboxPlacementPending} occupancyOnly=true" );
		_coverageTracePreviousMissing = missing;
		_coverageTraceMaximumMilliseconds = Math.Max( _coverageTraceMaximumMilliseconds,
			Stopwatch.GetElapsedTime( started ).TotalMilliseconds );
	}

	private object CaptureCoverageTrace()
	{
		var samples = new CoverageTraceSample[_coverageTraceCount];
		for ( var index = 0; index < samples.Length; index++ )
			samples[index] = _coverageTrace[(_coverageTraceCursor - _coverageTraceCount + index + _coverageTrace.Length) % _coverageTrace.Length];
		return new { Enabled = _coverageTraceEnabled, IntervalSeconds = 0.25,
			Scope = "27 LOD0 cells around player; occupancy only, not a surface-hole test",
			SamplesTaken = _coverageTraceSamples, SamplesWithMissingCoverage = _coverageTraceMissingSamples,
			RetainedSamples = samples, MaximumSampleMilliseconds = _coverageTraceMaximumMilliseconds };
	}

	[ConCmd( "voxel_coverage_trace" )]
	public static void SetCoverageTrace( bool enabled = true )
	{
		if ( !TryGetActiveManager( "coverage.trace", out var manager ) ) return;
		manager._coverageTraceEnabled = enabled;
		Log.Info( $"[VoxelWorld] coverage.trace enabled={enabled} reportCommand=voxel_coverage_report" );
	}

	[ConCmd( "voxel_coverage_report" )]
	public static void SaveCoverageReport()
	{
		if ( !TryGetActiveManager( "coverage.report", out var manager ) || manager._gpuMesher is null ) return;
		var center = manager.WorldToChunkCoordinate( manager.ActiveStreamingTarget.WorldPosition );
		var probes = new List<object>( 27 );
		var roots = new HashSet<GpuMeshRegionKey>();
		for ( var z = -1; z <= 1; z++ )
		for ( var y = -1; y <= 1; y++ )
		for ( var x = -1; x <= 1; x++ )
		{
			var key = new GpuMeshRegionKey( 0, center + new Vector3Int( x, y, z ) );
			roots.Add( CoverageAncestor( key, manager._targetVisualConfiguration.MaximumVisualLod ) );
			var covered = manager.FindCoverageAncestor( key, out var active );
			if ( covered )
			{
				var activeDescriptor = manager.CreateRegularDescriptor( active.Level, active.Coordinate, captureRegion: false );
				probes.Add( new { Key = key, ActiveAncestor = active,
					CurrentFieldResident = manager._gpuMesher.IsResident( activeDescriptor ),
					DrawEligible = manager._gpuMesher.IsRegionPresented( activeDescriptor ),
					Work = manager._gpuMesher.InspectRegionWork( activeDescriptor ) } );
				continue;
			}
			var ancestors = new List<object>( SupportedVisualLevelCount );
			for ( var level = 0; level <= manager._targetVisualConfiguration.MaximumVisualLod; level++ )
			{
				var ancestor = CoverageAncestor( key, level );
				var descriptor = manager.CreateRegularDescriptor( level, ancestor.Coordinate, captureRegion: false );
				ancestors.Add( new { Key = ancestor, Requested = manager._gpuMesher.Contains( descriptor ),
					Work = manager._gpuMesher.InspectRegionWork( descriptor ),
					Resident = manager._gpuMesher.IsResident( descriptor ), WaterReady = manager.IsChunkContentPrepared( descriptor ),
					HasAnyFinerCoverage = manager._gpuMesher.HasRenderDescendant( ancestor ),
					Desired = manager.IsLocalCoverageRequired( ancestor ), Queued = manager._coverageQueued.Contains( ancestor ),
					Exterior = manager.IsExteriorCoverageRequired( ancestor ),
					CoarseNeighborBlocked = manager.FindCoarseCoverageBlocker( ancestor, level + 1, out _ ) } );
			}
			var classification = VoxelChunk.ClassifyDensityRange( key.Coordinate, manager._appliedCellsPerAxis,
				manager._appliedCellSize, manager.CurrentField ).Classification;
			probes.Add( new { Key = key, Classification = classification.ToString(), Ancestors = ancestors } );
		}
		var coarseRoots = new List<object>( roots.Count );
		foreach ( var root in roots )
		{
			var pending = new Stack<GpuMeshRegionKey>();
			var missingRegions = new List<object>();
			pending.Push( root );
			var inspected = 0;
			while ( pending.Count > 0 && inspected++ < 512 && missingRegions.Count < 32 )
			{
				var key = pending.Pop();
				if ( manager.FindCoverageAncestor( key, out _ ) ) continue;
				if ( key.Level > 0 && manager._gpuMesher.HasRenderDescendant( key ) )
				{
					for ( var child = 7; child >= 0; child-- ) pending.Push( CoverageChild( key, child ) );
					continue;
				}
				var descriptor = manager.CreateRegularDescriptor( key.Level, key.Coordinate, captureRegion: false );
				missingRegions.Add( new { Key = key, Work = manager._gpuMesher.InspectRegionWork( descriptor ),
					WaterReady = manager.IsChunkContentPrepared( descriptor ),
					Desired = manager.IsLocalCoverageRequired( key ), Queued = manager._coverageQueued.Contains( key ),
					CoarseNeighborBlocked = manager.FindCoarseCoverageBlocker( key, key.Level + 1, out _ ) } );
			}
			coarseRoots.Add( new { Root = root, HasAnyFinerCoverage = manager._gpuMesher.HasRenderDescendant( root ),
				MissingRegions = missingRegions, Truncated = pending.Count > 0, InspectedNodes = inspected } );
		}
		var path = $"performance/coverage/{Guid.NewGuid():N}.json";
		global::Sandbox.FileSystem.Data.CreateDirectory( "performance/coverage" );
		global::Sandbox.FileSystem.Data.WriteAllText( path, System.Text.Json.JsonSerializer.Serialize( new
		{
			SchemaVersion = 1, CapturedAtUtc = DateTimeOffset.UtcNow, manager.CurrentField.WorldId,
			manager.CurrentField.Revision, PlayerPosition = manager.ActiveStreamingTarget.WorldPosition,
			Trace = manager.CaptureCoverageTrace(), Coverage = manager.CaptureLocalCoverage(), NearCells = probes,
			CoarseRoots = coarseRoots,
			Interpretation = "Any finer descendant is partial occupancy, not proof of full coarse-volume coverage. PotentiallySurfaceContaining is conservative, not proof of a visible hole."
		}, PerformanceJsonOptions ) );
		Log.Info( $"[VoxelWorld] coverage.report.saved path={(global::Sandbox.FileSystem.Data.GetFullPath( path ))}" );
	}
