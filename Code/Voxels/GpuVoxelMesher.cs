...
internal sealed partial class GpuVoxelMesher : IDisposable
{
	...
	// Sign carries the authoritative-correction flag; exponent owns the signature.
	private const uint TerrainRecordIdentitySignatureMask = 0x7f800000u;
	...
	public void Schedule( GpuSdfDescriptor descriptor, float playerRouteDistance, GpuMeshResidency residency )
	{
		...
		if ( _editPublicationOpen && (_editEpochChanged || _renderActive.Contains( descriptor.Key ) &&
			TerrainFieldChange.Intersects( descriptor.SamplingBounds, _editDependencyBounds )) )
	...
	public void ScheduleTransition( GpuTransitionDescriptor descriptor, float playerRouteDistance )
	{
		...
		if ( _editPublicationOpen && (_editEpochChanged || _transitionRenderActive.Contains( descriptor.Key ) &&
			TerrainFieldChange.Intersects( descriptor.SamplingBounds, _editDependencyBounds )) )
	...
	private void ProcessGpuRenderTick()
	{
		...
		var preferredService = PrioritizePlayerDetail && _placementQueued.Any( key => key.Level < 2 ) ? 0 : _nextGpuService;
	...
	private bool TrySubmitOuterCount()
	{
		...
		{
			if ( (activeLane.CountInFlight.Count > 0 && activeLane.CountInFlight[0].Descriptor.Key.Level >= 2) ||
				(activeLane.EmitInFlight.Count > 0 && activeLane.EmitInFlight[0].Descriptor.Key.Level >= 2) ) outerLanes++;
		}
		if ( outerLanes >= outerLaneLimit ) return false;
	...
	private DrawCommandCommitResult CommitDrawCommandsLocked( RenderCameraState state )
	{
		...
		if ( _visibilityCapacity > 0 && _fieldPresentationReady )
		{
			...
			foreach ( var arena in _arenas )
			{
				...
				if ( !state.DrawAttributes.TryGetValue( arena.Index, out var drawAttributes ) )
				{
					drawAttributes = new RenderAttributes();
	...
	private sealed class RenderCameraState : IDisposable, IHotloadManaged
	{
		...
		public void DisposeVisibility()
		{
			...
			DepthArenas = Array.Empty<GeometryArena>();
			Visibility?.Dispose();
			Visibility = null;
	...
	private sealed class TerrainDepthObject : SceneCustomObject
	{
		...
		public TerrainDepthObject( SceneWorld world, GpuVoxelMesher owner ) : base( world )
		{
			...
			// Each instance expands a contiguous block of canonical terrain triangle indices.
