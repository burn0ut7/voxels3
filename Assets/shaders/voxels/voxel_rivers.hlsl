...
// Endpoint texels contain XY position, bed depth and radius; water stays at seaLevel.
...
float4 SampleVoxelRiver( float2 position, float naturalHeight, float seaLevel )
{
	...
	while ( node < RiverNodeTexels )
	{
		...
		for ( uint index = 0u; index < count; index++ )
		{
			...
			float depth = start.z + t * (end.z - start.z);
			float bed = surface - depth * max( 0.0, 1.0 - normalized * normalized );
