...
float VoxelMaterialSurfaceHeight( float2 position )
{
	return SampleVoxelLandformHeight( position, VoxelMaterialTerrain,
		VoxelMaterialScales, VoxelMaterialRuggedness );
}

uint VoxelNodeMaterial( float depth, float surfaceHeight )
{
	if ( depth < 0.0 )
	{
		return (uint)VoxelMaterialIds.w;
	}
	if ( depth < VoxelMaterialRules.x && surfaceHeight >= VoxelSeaLevel )
	{
		return (uint)VoxelMaterialIds.x;
	}
...
float3 SampleVoxelMaterialColor( float3 position, float checker, float surfaceDepth )
{
	...
	for ( uint column = 0; column < 4; column++ )
	{
		...
		heights[column] = VoxelMaterialSurfaceHeight( xy );
	...
	for ( uint column = 0; column < 4; column++ )
	{
		...
		for ( uint z = 0; z < 2; z++ )
		{
			...
			uint id = VoxelNodeMaterial( depth, heights[column] );
