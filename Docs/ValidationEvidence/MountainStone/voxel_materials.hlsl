...
float3 SampleVoxelMaterialColor( float3 position, float checker, float surfaceDepth )
{
	...
	float weightSum = 0.0;
	// One base-cell XY footprint; two vertical nodes attached to each column.
	[unroll]
	for ( uint column = 0; column < 4; column++ )
	{
		// Preserve depth separately in each column. A shared lookup Z samples
		// underground strata in uphill neighbors and bands an untouched slope.
		float materialZ = (heights[column] - surfaceDepth) / VoxelMaterialRules.x;
		float nodeZ = floor( materialZ );
		float fractionZ = materialZ - nodeZ;
		[unroll]
		for ( uint z = 0; z < 2; z++ )
		{
			float depth = heights[column] - (nodeZ + (float)z) * VoxelMaterialRules.x;
			...
			{
				float weight = columnWeights[column] * (z == 0 ? 1.0 - fractionZ : fractionZ);
