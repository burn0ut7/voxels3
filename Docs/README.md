...
| What owns the first gathering/crafting loop? | [Opening survival](Architecture/DeadWakeSurvival.md): session-only offline candidate, inventory transactions, bounded resources and pending acceptance. |
...
| Why is LOD0 arrival delayed, and which external techniques should we prototype? | [LOD arrival research](Research/ChunkPerformanceOptimizationFindings.md#lod-arrival-research---2026-09-13): shipped-game and voxel-engine comparison, timing limitations, and ranked recovery, dependency-priority, and predictive-preparation experiments. |
...
| How do Godot terrain systems make texturing cheaper? | [Terrain texturing comparison](Research/TerrainTexturingComparison.md): pinned Terrain3D, HTerrain and Godot Voxel shaders; footprint-based detail, anti-repetition and cache tradeoffs; proposed experiments only. |
| How should convincing terrain relief be implemented? | [Terrain relief specification](Plans/TerrainReliefImplementation.md): matched height assets, one combined view intersection, starting parameters, lighting/depth gates and visual/performance acceptance; proposal only. |
| How can materials blend naturally without losing cell identity or small deposits? | [Material blending design](Research/TerrainMaterialBlending.md): live edge observation, natural references, geometry-independent coverage, relief mixing, grass agreement and proposed cache budgets; research only. |
| What did the four texturing prototypes achieve? | [Prototype results](Research/TerrainTexturingPrototypeResults.md): matched experiments, rejected variants, cache integration failures and restored-world checks. |
| Where did larger terrain rendering gains come from? | [High-gain investigation](Research/TerrainHighGainExperiments.md): whole-pass diagnostic limits, earlier distant geometry LOD, matched figure-eight results and visual tradeoffs. |
| Which changes improve textured terrain while standing still? | [Standstill optimization](Research/TerrainStandstillOptimization.md): material/lighting diagnostics, compressed grass, fog copy removal, shadow allocation and matched qualification. |
