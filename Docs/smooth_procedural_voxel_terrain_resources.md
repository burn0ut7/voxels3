...
| [Self-organizing tree models for image synthesis (Palubicki et al., 2009)](https://algorithmicbotany.org/papers/selforg.sig2009.html) | Separating local bud development, competition for light/space and resource allocation in the [shared growth authoring model](Architecture/TreeGrowth.md). | Our solver adopts this organization, not the paper’s equations, calibrated biology, collision guarantees or published visual/performance results. Species profiles remain authoring hypotheses. |
